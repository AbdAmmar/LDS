...
\begin{frame}[fragile]{Conditions (\texttt{if / elif / else})}
...
if [[ condition ]]; then
  ...
  \begin{itemize}
    ...
    \item négation : \texttt{! condition}

    \pause
    \item ``ET'' : condition1 \&\& condition2

    \pause
    \item ``OU'' : condition1 || condition2
  \end{itemize}
...
\begin{frame}[fragile]{Choix multiples \texttt{case}}
...
\begin{frame}[fragile]{Script Bash}
  ...
  \setlength\itemsep{1em}
    ...
    \item Exemple : fichier \texttt{hello.sh}
\begin{minted}{bash}
...
\end{minted}
    ...
    \begin{terminal}
      ...
      \prompt\ hello.sh: command not found
      }
...
\begin{frame}[fragile]{Permissions UNIX}
  ...
  \begin{itemize}
    ...
    \begin{center}
      \large
      \texttt{\textcolor{gray}{-} \textcolor{blue}{rwx} \textcolor{orange}{rw-} \textcolor{red}{r--}}
    \end{center}
...
\end{minted}
    ...
    \begin{terminal}
      \prompt\ \shcmd{additionner} 10 5

      \prompt\ La somme est : 15

      \prompt\ \shcmd{echo} \$resultat
...
\begin{frame}[fragile]{Rediriger le flux d'erreur}
  ...
  \begin{itemize}
    ...
    \item pour les erreurs, il faut ajouter \texttt{2} avant le chevron \texttt{>}
