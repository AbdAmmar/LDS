...
\begin{enumerate}
    ...
    Modifiez le script pour qu'il retourne un code d'erreur 1 et affiche une message d'erreur si aucun paramètre 
    n'est fourni.
...
\begin{enumerate}
    ...
    \item Rendez-vous dans le répertoire \texttt{TD4} et créez le sous-répertoire \texttt{Q3}.
    ...
    valeur numérique correspondante. Si l'argument fourni ne correspond à aucune couleur valide, le 
    script doit afficher le message d'erreur suivant :
    \texttt{Couleur invalide.}
...
\end{minted}
    ...
    \item Modifiez le code pour qu'il affiche la valeur de la résistance en utilisant 
    la fonction \texttt{obt\_val\_coul}.
    Testez votre script avec \texttt{calc\_resistance.sh "marron" "noir" "orange"}.
...
\section{Projet a rendre}

Placez-vous dans le répertoire \texttt{TD4} et créez le sous-répertoire \texttt{Q4}.
Placez-vous ensuite dans le répertoire \texttt{Q4}.
Dans ce répertoire, vouls allez creer XXX scripts a deposer sur arche.
Tous les scripts devraient etre executables avec le bon \textit{shebang}.
