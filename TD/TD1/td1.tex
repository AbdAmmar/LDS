...
\begin{enumerate}
    ...
    \begin{warningbox}
    Les fichiers et dossiers cachés ont des noms qui commencent par \texttt{.}. Ils sont
    invisibles, par exemple, avec la commande \texttt{ls}.
    \end{warningbox}

    \item Expliquez pourquoi la commande \texttt{man cd} ne fonctionne pas. Consultez 
          l'aide de la commande \texttt{cd}.

    % TODO: application avec ls -l
\end{enumerate}
...
\end{terminal}
    ...
    % TODO: un item pour utiliser \texttt{mv}
\end{enumerate}
...
\begin{terminal}
\prompt\ \shcmd{echo} "Rire, c'est vivre deux fois." \verb|>>| notes.txt

\prompt\ \shcmd{echo} "Après la pluie, le beau temps." \verb|>>| notes.txt
\end{terminal}
...
\begin{warningbox}
Les espaces peuvent être significatifs selon le contexte.
...
\begin{warningbox}
Les opérateurs \texttt{-gt}, \texttt{-lt}, \texttt{-eq} permettent de comparer
des entiers.
...
\begin{itemize}
    ...
    \item Modifiez la valeur de la variable \texttt{b} en lui ajoutant 10.
    \begin{terminal}
    \prompt\ \shcmd{b=}\$((b + 10))
    \end{terminal}
    ...
    \end{terminal}
    \begin{warningbox}
    Le code de sortie (\texttt{\$?}) suit la convention Unix : 0 indique un succès (\texttt{true}) et toute valeur non-nulle indique un échec (\texttt{false}). Cette convention est opposée à celle des langages de programmation où 0 évalue à \texttt{false} et 1 à \texttt{true} dans un contexte booléen.
    \end{warningbox}
\end{itemize}
