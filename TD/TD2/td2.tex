...
\begin{itemize}
    ...
    \begin{itemize}
        \item 
        Les guillemets simples \texttt{' '} désactivent l'interprétation de tous les caractères spéciaux.

        % ---

        \item
        Les guillemets doubles \texttt{" "} désactivent le globbing, mais permettent toujours
    ...
    \item 
    Créez 12 répertoires nommés \texttt{moi\_1}, \texttt{moi\_2}, \ldots, \texttt{moi\_12} en 
    ...
    \item 
    Sans changer de répertoire, créez 30 fichiers nommés \texttt{jour\_1.txt}, \texttt{jour\_2.txt}, 
...
\begin{itemize}
  \item 
  Le globbing (\texttt{*}, \texttt{?}, \texttt{[...]} dans le shell) sert à
  faire correspondre des motifs à des \textbf{noms de fichiers}.
  \item
  Les expressions régulières décrivent des motifs à l'intérieur du
...
\begin{itemize}
  \item
  Créez un sous-répertoire \texttt{Q3} dans \texttt{TD3} et placez-vous dedans.
  \begin{terminal}
  \prompt\ \shcmd{cd} \$HOME/TD3
...
\begin{itemize}
    ...
    \prompt\ \shcmd{grep} -E '000\$' donnees.txt
    \end{terminal}
    ...
    Par exemple, le regex \texttt{V*} signifie ``zéro ou plusieurs \texttt{V}''.
    ...
    Exemples avec \texttt{grep} :
    \begin{itemize}
        \item \texttt{grep '[0-9]' donnees.txt} : lignes contenant au moins un chiffre.
        \item \texttt{grep '\^{}[0-9][0-9][0-9]\$' donnees.txt} : lignes composées exactement de 3 chiffres.
    \end{itemize}
    ...
    \begin{terminal}
    \prompt\ \shcmd{grep} 'code postal: [0-9][0-9][0-9][0-9][0-9]' donnees.txt
    
    \prompt\ \shcmd{grep} -E 'code postal: [0-9]{5}' donnees.txt
    \end{terminal}

    \begin{infobox}
    \textbf{\texttt{grep} vs \texttt{grep -E}}
    \begin{itemize}
        \item \texttt{grep} (sans option) utilise des \textbf{expressions régulières basiques} (BRE).
              Certains symboles doivent être échappés avec \texttt{\textbackslash} pour avoir leur sens regex :
              \begin{itemize}
                \item \texttt{\textbackslash+} : une ou plusieurs fois
                \item \texttt{\textbackslash?} : zéro ou une fois
                \item \texttt{\textbackslash|} : alternative (OU)
                \item \texttt{\textbackslash( ... \textbackslash)} : groupement
                \item \texttt{\textbackslash\{m,n\textbackslash\}} : répétitions
              \end{itemize}
        
        \item \texttt{grep -E} active les \textbf{expressions régulières étendues} (ERE) : on peut utiliser
              directement \texttt{+}, \texttt{?}, \texttt{|}, \texttt{( )}, \texttt{\{ \}} sans antislash.
              \begin{itemize}
                \item \texttt{grep -E 'a+'} : une ou plusieurs lettres \texttt{a}
                \item \texttt{grep -E 'foo|bar'} : \texttt{foo} ou \texttt{bar}
                \item \texttt{grep -E '(ab){3}'} : \texttt{ab} répété 3 fois
              \end{itemize}
        
        \item Les éléments comme \texttt{.}, \texttt{*}, \texttt{[...]} , \texttt{[\^{}...]}, \texttt{\^{}} et \texttt{\$}
              fonctionnent de la même manière avec \texttt{grep} et \texttt{grep -E}.
    \end{itemize}
    \end{infobox}
    ...
    \begin{infobox}
    En shell, les guillemets simples \texttt{' '} et les guillemets doubles \texttt{" "}
    n'ont pas le même comportement :
    \begin{itemize}
        \item Les guillemets simples \texttt{'motif'} passent le texte tel quel
              à la commande : aucune expansion de variable, pas de \texttt{\$(...)}.
              C'est ce que l'on préfère pour écrire des regex avec \texttt{grep}, \texttt{sed}, etc.
        \item Les guillemets doubles \texttt{"motif"} laissent encore le shell développer
              les variables (\texttt{\$}) et les substitutions de commande (\texttt{\$(\dots)}).
    \end{itemize}
    \end{infobox}
  
    % ---

  \item Affichez toutes les lignes contenant \texttt{tel:} suivi d'un chiffre (\texttt{0} à \texttt{9}) en première position après \texttt{tel: }.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}tel: [0-9]' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes contenant \texttt{PROD-} suivi de \texttt{ABC} ou \texttt{XYZ}, suivi de \texttt{-} et de n'importe quels caractères.
        (Nous utiliserons ici \texttt{. *} pour « zéro ou plusieurs caractères quelconques ». Nous verrons plus tard une meilleure façon d'écrire cela.)
  \begin{terminal}
  \prompt\ \shcmd{grep} -E 'PROD-(ABC|XYZ)-.*' donnees.txt
  \end{terminal}

    % ---

    \item Cherchez toutes les lignes contenant des heures (format \texttt(hh:mm:ss))
    \begin{terminal}
    \prompt\ \shcmd{grep} '..:..:..' donnees.txt

    \prompt\ \shcmd{grep} '[0-9]\{2\}:[0-9]\{2\}:[0-9]\{2\}' donnees.txt

    \prompt\ \shcmd{grep} '\([01][0-9]\|2[0-3]\):[0-5][0-9]:[0-5][0-9]' donnees.txt
    \end{terminal}
...
\begin{infobox}
\begin{itemize}
  \item \texttt{\^{}} (début de ligne) : la regex doit commencer en début de ligne. \\
        Exemple : \texttt{\^{}ERROR} : lignes qui commencent par \texttt{ERROR}.
  \item \texttt{\$} (fin de ligne) : la regex doit finir en fin de ligne. \\
        Exemple : \texttt{\.com\$} : lignes qui se terminent par \texttt{.com}.
\end{itemize}
\end{infobox}

\begin{itemize}
  \item
  Affichez toutes les lignes de \texttt{donnees.txt} qui commencent par \texttt{ERROR}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}ERROR' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes qui commencent par \texttt{INFO}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}INFO' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes qui se terminent par \texttt{.com}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '.com\$' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes qui se terminent par un chiffre (\texttt{0} à \texttt{9}).
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '[0-9]\$' donnees.txt
  \end{terminal}
\end{itemize}

\subsection{Quantificateurs : \texttt{+}, \texttt{?}, \texttt{\{m,n\}} (avec \texttt{grep -E})}

\begin{infobox}
Avec \texttt{grep -E}, on dispose de quantificateurs pratiques :
\begin{itemize}
  \item \texttt{+} : « une ou plusieurs occurrences » de l'élément précédent.
  \item \texttt{?} : « zéro ou une occurrence » de l'élément précédent.
  \item \texttt{\{m\}} : exactement \texttt{m} occurrences.
  \item \texttt{\{m,n\}} : entre \texttt{m} et \texttt{n} occurrences.
\end{itemize}
Exemples : \texttt{[0-9]\{5\}} : exactement 5 chiffres ; \texttt{[A-Z]+} : une ou plusieurs majuscules.
\end{infobox}

\begin{itemize}
  \item Affichez les lignes contenant un code postal à 5 chiffres (lignes commençant par \texttt{code postal: } suivi d'exactement 5 chiffres).
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}code postal: [0-9]{5}\$' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes contenant une séquence d'au moins 3 chiffres consécutifs.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '[0-9]{3,}' donnees.txt
  \end{terminal}

  \item Affichez les lignes de date au format \texttt{2023-03-01} (année sur 4 chiffres, tiret, mois sur 2 chiffres, tiret, jour sur 2 chiffres).
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}date: [0-9]{4}-[0-9]{2}-[0-9]{2}\$' donnees.txt
  \end{terminal}

  \item Affichez les lignes de date au format \texttt{01/03/2023} (jour/mois/année, avec des chiffres et des slashs).
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}date: [0-9]{2}/[0-9]{2}/[0-9]{4}\$' donnees.txt
  \end{terminal}
\end{itemize}
...
Avec \texttt{grep -E} :
\begin{itemize}
  \item \texttt{A|B} : correspond à \texttt{A} \emph{ou} \texttt{B}.
  \item \texttt{( ... )} : permet de grouper des parties d'expression régulière.
\end{itemize}
Exemples : \texttt{(INFO|WARN|ERROR)} : l'une des trois chaînes. \\
\texttt{PROD-(ABC|XYZ)-[0-9]+} : \texttt{PROD-ABC-} ou \texttt{PROD-XYZ-} suivi de chiffres.
\end{infobox}

\begin{itemize}
  \item Affichez toutes les lignes de logs dont le niveau est \texttt{ERROR} ou \texttt{WARN}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}(ERROR|WARN)' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes contenant un e-mail de domaine \texttt{example.com} \emph{ou} \texttt{example.org}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E 'example.(com|org)' donnees.txt
  \end{terminal}

  \item Affichez toutes les références produit \texttt{ref:} dont le code commence par \texttt{PROD} et contient \texttt{ABC} ou \texttt{XYZ}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^{}ref: PROD-(ABC|XYZ)-[0-9]+' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes dont le module est \texttt{auth} ou \texttt{payments} (champ \texttt{module=...} dans les lignes de logs).
  \begin{terminal}
  \prompt\ \shcmd{grep} -E 'module=(auth|payments)' donnees.txt
  \end{terminal}
\end{itemize}

\subsection*{Bonus : classes négatives, \texttt{grep -v} et introduction à \texttt{sed}}

\subsubsection*{Classes négatives \texttt{[\^{}...]}} 

\begin{infobox}
Dans une regex, une classe de caractères négative \texttt{[\^...]} signifie : \\
« un caractère qui n'est \textbf{pas} dans la liste ». \\
Exemples : 
\begin{itemize}
  \item \texttt{[\^0-9]} : un caractère qui n'est pas un chiffre.
  \item \texttt{[\^aeiou]} : une lettre qui n'est pas une voyelle parmi \texttt{a,e,i,o,u}.
\end{itemize}
\end{infobox}

\begin{itemize}
  \item Affichez toutes les lignes commençant par \texttt{tel: } suivies d'un caractère qui n'est \textbf{pas} \texttt{+}
        (c'est-à-dire les numéros de téléphone locaux, et non ceux au format international \texttt{+33 ...}).
  \begin{terminal}
  \prompt\ \shcmd{cd} \$HOME/TD3/Q3
  \prompt\ \shcmd{grep} -E '\^tel: [\^+]' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes de code postal dont le premier chiffre (après \texttt{code postal: }) n'est \textbf{pas} \texttt{7}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '\^code postal: [\^7]' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes contenant au moins un caractère qui n'est pas une lettre minuscule ni un espace.  
        (On cherche ici des lignes « techniques » : chiffres, signes de ponctuation, etc.)
  \begin{terminal}
  \prompt\ \shcmd{grep} -E '[\^a-z ]' donnees.txt
  \end{terminal}
\end{itemize}

\subsubsection*{\texttt{grep -v} : inverser la sélection}

\begin{infobox}
\texttt{grep -v motif} affiche les lignes qui \textbf{ne correspondent pas} au motif. \\
C'est l'inverse de \texttt{grep motif}.
\end{infobox}

\begin{itemize}
  \item Affichez toutes les lignes qui \textbf{ne} contiennent \textbf{pas} le mot \texttt{email}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -v 'email' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes qui ne sont pas des logs (c'est-à-dire qui ne commencent ni par \texttt{INFO}, ni par \texttt{WARN}, ni par \texttt{ERROR}).
  \begin{terminal}
  \prompt\ \shcmd{grep} -Ev '\^(INFO|WARN|ERROR)' donnees.txt
  \end{terminal}

  \item Affichez toutes les lignes de \texttt{donnees.txt} qui \textbf{ne} contiennent \textbf{pas} d'adresse e-mail \texttt{@entreprise.com}.
  \begin{terminal}
  \prompt\ \shcmd{grep} -Ev 'entreprise\.com' donnees.txt
  \end{terminal}
\end{itemize}

\subsubsection*{Introduction à \texttt{sed} pour de simples remplacements}

\begin{infobox}
\texttt{sed} est un éditeur de flux, très puissant. \\
Commande de base pour un remplacement :
\begin{itemize}
  \item \texttt{sed 's/ancien/nouveau/'} remplace la \textbf{première} occurrence de \texttt{ancien}
        par \texttt{nouveau} sur chaque ligne.
  \item \texttt{sed 's/ancien/nouveau/g'} remplace \textbf{toutes} les occurrences sur chaque ligne.
\end{itemize}
